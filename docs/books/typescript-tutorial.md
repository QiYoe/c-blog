...
而他们的定义文件，则在 [TypeScript 核心库的定义文件中](https://github.com/Microsoft/TypeScript/tree/main/src/lib)。
...
[之前学习过](#对象的类型—接口)，接口（Interfaces）可以用于对「对象的形状（Shape）」进行描述。
...
上例中，我们使用了[之前提到过的数组泛型](#数组泛型)来定义返回值的类型。
...
[之前学习过](#函数的类型)，可以使用接口的方式来定义一个函数需要符合的形状：
```ts
interface SearchFunc {
  (source: string, subString: string): boolean;
}
